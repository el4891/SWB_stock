...
#        tmpMonth = nowMonth
    ...
    if price[i] < pingjunchengben * (1 - zhiyinzhisunbili):#如果持有标志为1则再把剩余的钱买入
        tmpStockNum = int((savedMoney + restMoney)/(1+fee) / price[i] / fene * (1 + buytimes / 6) / 100) * 100
        ...
        if tmpStockNum != 0:#购买一份了，那就买买买
            ...
            buyShow.append(price[i])
            buyDateShow.append(datetime.datetime.strptime(date[i][0:10], '%Y-%m-%d').date())
    elif price[i] < jinqidingbu * (1 - zhiyinzhisunbili):#如果持有标志为0, 卖掉
        restMoney += (stockNum / 100 / fene * 100 * price[i] * (1-fee))
        stockMoney = stockMoney - stockNum / 100 / fene * 100 * price[i]
        zongtourujine = (zongtourujine - stockNum / 100 / fene * 100 * price[i]) * (1 + (jinqidingbu / pingjunchengben-1) / (fene -1 ))
        jinqidingbu = jinqidingbu * (1 - zhiyinzhisunbili)
        buytimes = 0
