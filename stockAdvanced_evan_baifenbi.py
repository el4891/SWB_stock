...
zhiyinzhisunbili = 0.26
fene = 8

pingjunchengben = price[22] * (1 + zhiyinzhisunbili)
zongtourujine = 0
...
#        tmpMonth = nowMonth
    ...
    if stockNum > 0:
        pingjunchengben = zongtourujine / stockNum
    ...
    if price[i] < pingjunchengben * (1 - zhiyinzhisunbili):#如果持有标志为1则再把剩余的钱买入
        tmpStockNum = int((savedMoney + restMoney)/(1+fee) / price[i] / 100 / fene) * 100
        if tmpStockNum != 0:#购买一份了，那就买买买
            stockNum += tmpStockNum
            stockMoney += (tmpStockNum * price[i])
            zongtourujine += (tmpStockNum * price[i])
            restMoney = restMoney + savedMoney - tmpStockNum * price[i] - tmpStockNum * price[i] * fee
            savedMoney = 0#存起来的钱清零
            buyShow.append(price[i])
            buyDateShow.append(datetime.datetime.strptime(date[i][0:10], '%Y-%m-%d').date())
    elif price[i] > pingjunchengben * (1 + zhiyinzhisunbili):#如果持有标志为0, 卖掉
        restMoney += (stockNum / 100 / fene * 100 * price[i] * (1-fee))
        stockMoney = stockMoney - stockNum / 100 / fene * 100 * price[i]
        zongtourujine = (zongtourujine - stockNum / 100 / fene * 100 * price[i]) * (price[i] / pingjunchengben)
        stockNum = stockNum - stockNum / 100 / fene * 100
...
#plt.show()
wb.save(name + '_' + str(datetime.datetime.now().strftime('%d_%H_evan_baifenbi')) + '.xls')
